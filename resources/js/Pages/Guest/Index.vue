<template>
    ...
    <AuthenticatedLayout>
        <div class="pl-12 pr-4 py-4">
            <DataTable :data="props.guests.data" :columns="columns" :pagination="props.guests.meta"
...
const columns = ref([
    ...
    { label: 'TYPE', key: 'types.name' },
    { label: 'CHECKED IN MOTEL', key: 'users.motels.0.motel_name' }
])
...
const handleSubmit = async () => {
    const result = await storeItem('guest', formData)
    if (result.success) {
...
const handleUpdate = async () => {
    const result = await updateItem(`guest/${itemId.value}`, formData)
    if (result.success) {
        router.get(route('guest.index'), {
            preserveState: true,
            preserveScroll: true
...
const editData = async (id) => {
    itemId.value = id
    const result = await fetchItem(`guest/${id}`)
    if (result.success) {
...
const limitQuery = (name, e) => {
    ...
    router.get(route('guest.index', params))
}
...
const deleteData = (id) => {
    if (confirm('Are you sure you want to delete this guests?')) {
        router.delete(route('guest.destroy', id), {
            preserveState: true
        })
    }
