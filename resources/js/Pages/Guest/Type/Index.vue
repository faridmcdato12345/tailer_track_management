...
const handleSubmit = async () => {
    const result = await storeItem('guest_type', formData)
    if (result.success) {
        router.get(route('guest_type.index'), {
            preserveState: true,
            preserveScroll: true
        })
...
const handleUpdate = async () => {
    const result = await updateItem(`guest_type/${itemId.value}`, formData)
    if (result.success) {
        router.get(route('guest_type.index'), {
            preserveState: true,
            preserveScroll: true
...
const editData = async (id) => {
    itemId.value = id
    const result = await fetchItem(`guest_type/${id}`)
    if (result.success) {
...
const limitQuery = (name, e) => {
    ...
    router.get(route('guest_type.index', params))
}
...
const deleteData = (id) => {
    if (confirm('Are you sure you want to delete this type?')) {
        router.delete(route('guest_type.destroy', id), {
            preserveState: true
        })
    }
